...
The project includes a complete **Docker Compose** setup for rapid local development and testing without requiring .NET SDK installation.
...
|---------|------|-------------|
| ApiServiceAtFront | 5000 | Frontend API service |
| ApiService | 5001 | Backend API service with MongoDB |
| Pulsar Broker | 6650 | Message queue |
| Pulsar Admin | 8080 | Pulsar web UI |
...
docker-compose logs apiservice -f
docker-compose logs apiserviceatfront -f

# Execute command in running container
docker exec dotnet-apiservice curl http://localhost:8080/
...
## Traditional Setup with .NET Aspire

Alternatively, you can run the project using .NET Aspire AppHost (requires .NET 10 SDK).

### Prerequisites
- .NET 10 SDK
- Docker (for MongoDB and Pulsar)
- 4GB RAM
...
# Then run:
dotnet run --project DotnetTelemetryPlayground.AppHost
```

This starts the Aspire orchestration which manages all services.
...
**Option 2: .NET Aspire (Full Development Experience)**
- .NET 10.0 SDK or later
- Visual Studio 2022+ or VS Code with C# extensions
- Docker or Podman (for Pulsar, MongoDB, telemetry backends)

### Quick Start: Docker Compose

**Fastest way to get everything running in minutes:**
...
- **Pulsar UI**: http://localhost:8080/admin/v2/overview
- **MongoDB**: localhost:27017

See [DOCKER_SETUP.md](DOCKER_SETUP.md) for detailed Docker Compose documentation.

### Full Development: .NET Aspire

**For complete development experience with debugging and monitoring:**
...
See **Quick Start** section above for Docker Compose commands.
...
3. **Access the Aspire Dashboard**
   ...
   - Default URL: http://localhost:18888 (or shown in console)

### Testing Telemetry Scenarios

#### Testing with Docker Compose

After starting with `docker-compose up -d`, test the APIs:

```bash
# Get weather data from frontend service
...
curl -X PUT http://localhost:5000/api/weather \
  -H "Content-Type: application/json" \
  -d '{
    "date": "2025-12-17",
    "temperatureC": -15,
...
# View Pulsar topics
curl http://localhost:8080/admin/v2/persistent/public/default/partitions

# View MongoDB data
mongosh --authenticationDatabase admin -u admin -p password localhost:27017
```

#### Testing with Aspire

The Aspire Dashboard automatically captures and displays:
...
**Test Endpoints** (when using Aspire, use HTTPS ports shown in dashboard):

1. **Service-to-Service Communication**
   ```bash
   curl https://localhost:7055/api/weather
   ```
   - Frontend calls backend API
   - Observe HTTP trace propagation
   - View parent-child span relationships

2. **Async Message Patterns**
   ```bash
   curl -X POST https://localhost:7055/api/weather \
     -H "Content-Type: application/json" \
     -d '{
       "date": "2025-12-17",
       "temperatureC": -10,
   ...
   ```
   - Messages published to Pulsar with trace context
   - Consumer processing with parent trace context
   - End-to-end message flow visibility

3. **Resilience Testing**
   - Stop backend service to trigger retry logic
   - Observe resilience patterns in traces
...
**With Aspire:**
- Use the integrated Aspire Dashboard (opens automatically)
- View metrics, traces, and logs in the UI
...
```
dotnet-telemetry-playground/
├── DotnetTelemetryPlayground.ApiService/          # Backend API (weather processor)
│   ├── Dockerfile                                  # Production-ready multi-stage Docker build
│   ├── Modules/Weather/                            # Business logic modules
│   ├── Program.cs                                  # Service configuration
│   └── appsettings.json                           # Configuration with connection strings
├── DotnetTelemetryPlayground.ApiServiceAtFront/   # Frontend API (request aggregator)
│   ├── Dockerfile                                  # Production-ready multi-stage Docker build
│   ├── Modules/Weather/                            # API endpoints
│   ├── Program.cs                                  # Service configuration
│   └── appsettings.json                           # Configuration
├── DotnetTelemetryPlayground.AppHost/              # Aspire orchestration (optional)
├── DotnetTelemetryPlayground.ServiceDefaults/      # Shared telemetry & resilience config
├── DotnetTelemetryPlayground.Shared/               # Shared models and interfaces
├── Custom.Client/                                  # Pulsar and MongoDB client extensions
├── Custom.Hosting/                                 # Custom Aspire resource builders
├── docker-compose.yml                              # Docker Compose orchestration
├── .dockerignore                                   # Docker build optimization
├── DOCKER_SETUP.md                                 # Comprehensive Docker documentation
├── readme.md                                       # This file
└── Example.Clients/                                # Example test clients (C#, TypeScript)
```

## Deployment Options

### Development
- **Docker Compose**: Local development with all dependencies (recommended)
- **Aspire AppHost**: Full IDE integration with debugging and monitoring

### Production
- **Containerized Deployment**: Use provided Dockerfiles with orchestration platforms:
...
- Remove `AppHost` project (Aspire is development-only)
- Deploy services independently using Dockerfiles
- Configure external message queue and database services
- Set up external OpenTelemetry collector (e.g., DataDog, Jaeger, New Relic)
...
services:
  apiservice:
    ports:
      - "5010:8080"  # Change from 5001 to 5010
```
...
Check that all services are listed in Aspire Dashboard with "Healthy" status.
...
- New telemetry scenarios
- Additional example clients
...
- [.NET Observability](https://docs.microsoft.com/en-us/dotnet/core/diagnostics/observability)
